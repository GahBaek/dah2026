...
\textbf{실험 결과:} 기체의 \textbf{실제 경로 이탈이 최대 34.99 m까지 발생하여 공격자가 주입한 기만 오차값과 거의 1:1로 일치}하는 결과를 보인다. 수직 관성 및 기압 융합 가용성이 낮아 내부 필터 (EKF)의 이상치 저항력이 약한 지상 이동 로봇의 특성상 GPS 데이터를 전적으로 신뢰하게 되며, 이는 Copter 호버링 제어 시의 EKF 완전 저항 성능과 정반대로 기만이 실제 이탈 피해로 고스란히 전이되는 정직한 취약성을 입증한다.
...
T-UAV-7 미션/파라미터 변조 & UAV & 지오펜스 침범 유도 (0.175 s); 복구 및 차단 (ASR 1.0$\to$0.0) & \texttt{push\_mission\_param\_emergent/} \\
...
T-SAT-1 링크 열화 (지연/손실) & SATCOM & Heartbeat 1.13$\to$0.67 Hz 저하, RTT 지연 (동어반복) & \texttt{build\_datalink/} \\
...
\begin{center}
\fbox{\parbox{0.95\textwidth}{\textbf{정직성 경계.} 아래 KC-3/KC-4는 이미 측정된 primitive들의 도트린 수준 합성 (design-level composition)이며 신규 라이브 SITL 런이 \textbf{아니다}. 각 단계는 앞서 측정한 창발 지표에 매핑되고, 시퀀싱만 실제 사건 도트린을 따른다. KC-A/KC-B (실측 합성)와는 명확히 구분된다.}}
\end{center}
...
\begin{itemize}[leftmargin=*]
    ...
    ① 데이터링크 DoS 및 재밍을 통해 통신 블랙아웃을 유도하여 기체의 링크 상실 페일세이프를 강제 촉발한다 (실측 발동 시간 $\approx$14.6 s). 
